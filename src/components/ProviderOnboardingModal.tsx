            ...
            {[
              ...
              <div key={item.step} className="flex gap-4 group">
                <div className="flex flex-col items-center">
                  <div className={`w-10 h-10 rounded-[14px] flex items-center justify-center transition-all ${currentStep === item.step
                  ...
                  <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {filteredServices.map(s => (
                        <div key={s._id} onClick={() => { setSelectedService(s); clearError('service'); }} className={`border-2 rounded-[32px] p-6 flex items-center justify-between cursor-pointer transition-all duration-300 ${selectedService?._id === s._id ? "border-blue-600 bg-blue-50/30 ring-8 ring-blue-600/5" : "border-slate-50 bg-slate-50/50 hover:border-blue-200"}`}>
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${selectedService?._id === s._id ? "bg-blue-600 text-white rotate-6" : "bg-white text-slate-400"}`}><Settings size={22} /></div>
                            <div><p className="font-black text-slate-900">{s.name}</p></div>
                          </div>
                          {(selectedService?._id === s._id) && <div className="bg-blue-600 text-white w-6 h-6 rounded-full flex items-center justify-center shadow-lg"><Check size={14} /></div>}
                        </div>
                      ))}
                    </div>
