                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-slate-800">
                      <Calendar size={12} className="text-slate-400 shrink-0" />{booking.date}
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] font-semibold text-slate-500">
                      <Clock size={11} className="text-slate-400 shrink-0" />{booking.slot.start} – {booking.slot.end}
                    </div>
                  </div>
                  ...
                  <div>
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border whitespace-nowrap ${sc.cls}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`} />
                      {sc.label}
                    </span>
                    {booking.cancellationReason && (
                      <p className="text-[9px] text-rose-400 font-medium mt-1 truncate max-w-[120px]" title={booking.cancellationReason}>{booking.cancellationReason}</p>
                    )}
                  </div>
                  ...
                  <div className="relative shrink-0">
                    ...
                    {openMenuId === booking._id && (
                      <div className="absolute right-0 top-10 z-50 w-44 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
                        {(booking.status === "confirmed" || booking.status === "in_progress" || booking.status === "completed_pending_payment" || booking.status === "completed") && (
                          <button onClick={() => { handleOpenChat(booking._id); setOpenMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-slate-50">
                            <MessageSquare size={15} /> Chat
                          </button>
                        )}
                        <button onClick={() => { setReportBooking(booking); setOpenMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-rose-600 hover:bg-rose-50 font-medium">
                          <ShieldAlert size={15} /> Report Provider
                        </button>
                        {canAct && (
                          <>
                            <button onClick={() => { handleRescheduleClick(booking); setOpenMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-slate-50">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[32px] w-full max-w-sm shadow-2xl border border-slate-100 p-8 animate-in fade-in zoom-in-95 duration-200 space-y-6">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-slate-400 font-medium leading-relaxed">Are you sure you want to cancel this booking? This action cannot be undone.</p>
            </div>
            <div className="space-y-2">
              ...
              />
              {cancelReasonError && <p className="text-red-500 text-[10px] font-semibold mt-1 ml-1">{cancelReasonError}</p>}
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setCancelBookingId(null)} className="flex-1 bg-slate-50 hover:bg-slate-100 border border-slate-100 text-slate-700 py-3 rounded-xl text-xs font-bold transition-all">Back</button>
              <button type="button" onClick={handleConfirmCancel} disabled={isCancelling} className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5">
                {isCancelling && <Loader2 size={12} className="animate-spin" />} Cancel Booking
              </button>
            </div>
          </div>
        </div>
