            ...
            <div className="flex items-start gap-6">
              <div className="w-16 h-16 rounded-2xl bg-white border border-slate-100 p-1 shrink-0 shadow-sm">
                {booking.userId?.profilePhoto ? (
                  <img src={booking.userId.profilePhoto} alt="" className="w-full h-full rounded-xl object-cover" />
                ) : (
                  <img src={`https://api.dicebear.com/7.x/initials/svg?seed=${booking.userId?.name}`} alt="" className="w-full h-full rounded-xl" />
                )}
              </div>
              <div className="space-y-4 flex-1">
                <div>
                  <h4 className="text-lg font-black text-slate-900">{booking.userId?.name}</h4>
                </div>
